...
```mermaid
flowchart TD
    A["<b>01</b> 느린 클라이언트가<br/>공유 스레드를 붙잡아<br/>다른 방까지 지연"] --> B["해결: 점유 시간에 상한을 두고<br/>못 따라오는 세션은 끊는다"]
    B --> C["끊어도 되는 근거:<br/>클라이언트가 재접속하면<br/>전체 재조회로 복구된다"]
    C --> D{"그런데 그 복구가<br/>전원 <b>동시에</b> 일어나면?"}
    D --> E["<b>02</b> 배포 = 전원 동시 절단<br/>→ 재시도 폭풍<br/>→ 콜드 서버에 최대 스파이크"]
...
flowchart LR
    P["메시지 발행"] --> B["브로커"]
    B --> Q["<b>공유</b> 배달 스레드풀<br/>스레드 코어×2 · 큐 무한"]
    Q --> S1["A방 세션 (정상)"]
    Q --> S2["B방 세션 (느림)"]
